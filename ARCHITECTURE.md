...
| **API Handler**                    | API Gateway              | Serves plugin requests: session status (`GET /status`), OneDrive auth URL (`GET /onedrive/auth-url`), list pending files, issue pre-signed S3 URLs, acknowledge delivery                             |

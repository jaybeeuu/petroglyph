...
- **Manual "Sync Now" command** — available via command palette and settings. Calls `POST /sync/run` first (dispatches an async Graph delta query — returns 201 with a `jobId`; the sync-worker Lambda processes the job in the background), then pages through `GET /files/changes` as above.

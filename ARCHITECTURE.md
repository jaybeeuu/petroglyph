...
| **DynamoDB**                           | File records (path, S3 key, sequence position for change feed), Graph delta token for sync runs, subscription metadata, sync profile records (including `oneDriveConnected` flag read by `GET /status`), and sync job records (`jobId`/`profileId`/`sourceFolderPath`/`userId`, status, fileCount, error tracking). The `sync_jobs` table is stream-enabled (`NEW_IMAGE`) and serves as the dispatch outbox — the API writes it once, and a relay Lambda fans records out to SQS. |

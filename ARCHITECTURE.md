...
The `package` and `deploy` jobs currently wire only the API and ingest-onedrive zips. The sync-worker and sync-relay have no `package` script or CD zip upload yet, so Terraform's `sync_worker_zip_*` / `sync_relay_zip_*` variables default to empty and those Lambdas are **not** created by CD until an operator stages their zips (see [docs/ops.md](docs/ops.md#lambda-packaging-esm)) and passes the S3 keys to `terraform apply`.
...
| Object versioning     | `ObjectStore.put` keeps `versionId?`; `stage()` drops it     | `packages/infra/s3.tf` does not enable bucket versioning on `staged_pdfs`, so S3 never returns a `VersionId` in production and the staging caller has no use for one. The generic `ObjectStore` port stays S3-faithful so version tracking can be enabled later without re-adding the field to the staging contract.                                                                                                                                                                                                               |
| Staged content type   | `MimeType` union, produced by `detectType`                   | `StageInput.contentType` is never a caller-chosen string — it is always the value `detectType` verified from the body's magic bytes. A closed `MimeType` union names the accepted types (PDF today) and keeps `null` ("unrecognised") outside the alias; adding an accepted type extends the magic-byte table and the union together.                                                                                                                                                                                              |
...
| Delta change routing  | Never advance the token on an unresolved path                | Graph's delta contract does not guarantee `parentReference.path` (it tracks items by id), so a page can contain items whose location cannot be derived. Advancing the token past one would skip it silently and permanently, so a non-zero unresolved-path count returns `outcome: "failed"` and the token is not written. A drive-root item has no folder segment for a staging key: it is counted and skipped, not fatal.                                                                                                        |
| Delta item shape      | Discriminate on facets, never fabricate an omitted field     | Graph's `changeType` is a change-notification-only property, absent from delta responses, so delta items are keyed by their `file`/`folder`/`deleted` facets; a deleted folder carries both `folder` and `deleted`, so `deleted` wins. Fields the delta omits stay absent — `name` is never substituted with the item id, because a delete that omits `name` would then derive a removal key that matches nothing. The walk carries no created-vs-updated distinction; that is left to the version dimension (petroglyph-j1gn.17). |
| Delta walk completion | Fail when a page carries neither link                        | A page that carries neither `@odata.deltaLink` nor `@odata.nextLink` never reached a terminal change token, so there is nothing to advance. Returning `continued` would report the walk as complete with the stored token left where it was, so the walk returns `outcome: "failed"` instead.                                                                                                                                                                                                                                      |

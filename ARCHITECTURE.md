...
| **DynamoDB**                           | File records (path, S3 key, sequence position for change feed), Graph delta token for sync runs, subscription metadata, sync profile records (including `oneDriveConnected` flag read by `GET /status`), and sync job records (status, fileCount, error tracking for async dispatch).                                     |
...
| **SQS Sync-Jobs Queue**         | Decouples the API handler from the sync worker. `POST /sync/run` enqueues a job and returns immediately; the worker processes asynchronously. |
...
| Async sync dispatch   | `POST /sync/run` returns 201 + jobId           | Synchronous delta walk timed out Lambda at 30+ s with many files; async dispatch via SQS decouples API response time from Graph query duration and enables job-status tracking.                                                                |

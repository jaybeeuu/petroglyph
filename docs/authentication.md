...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reauthorizationRequired` | Lambda reads the user's refresh token from DynamoDB, performs token refresh, stores rotated tokens back to DynamoDB, then calls `POST /subscriptions/{id}/reauthorize`. If reauthorization fails — e.g. 308 Permanent Redirect, a known Microsoft EU-datacenter issue for personal OneDrive — it falls back to deleting the subscription (`DELETE /subscriptions/{id}`, ignoring 404) and re-registering a fresh one, then marks the user connected. Only if the recreate also fails does it mark OneDrive as `reconnect_required` in DynamoDB. |
| `subscriptionRemoved`     | Marks OneDrive as `reconnect_required` in DynamoDB immediately.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

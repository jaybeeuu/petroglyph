...
CloudEvents 1.0.2 envelope machinery shared by every producer and consumer, plus the event-log
transport. It holds **no domain vocabulary** — events are declared with their domain (today,
`packages/staging-contracts`). For the taxonomy and why it exists, see
...
| `id`              | Unique within `source`                                                   | `source` + `id` is the idempotency anchor. `id` is required and producer-supplied — the envelope schema deliberately does not generate one, because a random default would silently defeat the anchor rather than fail. Producers use deterministic ids (`<profileId>:<itemId>:<changeType>`), so a re-send for the same logical event may repeat `source` + `id`: the log's put-if-absent condition suppresses the duplicate write, and consumers dedupe on the same anchor. |
...
Declaring the event with `cloudEventSchema` and literal `type`/`dataschema` values is the
recommended way to guarantee conformance; it is not mandated — the standard is the contract, not
the helper.
